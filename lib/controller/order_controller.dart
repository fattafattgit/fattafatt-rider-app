...
import 'package:fattafatt_rider/data/model/body/record_location_body.dart';
import 'package:fattafatt_rider/data/model/body/update_status_body.dart';
import 'package:fattafatt_rider/data/model/response/ignore_model.dart';
import 'package:fattafatt_rider/data/model/response/order_details_model.dart';
import 'package:fattafatt_rider/data/model/response/order_model.dart';
import 'package:fattafatt_rider/data/repository/order_repo.dart';
...
class OrderController extends GetxController implements GetxService {
  ...
  OrderModel _optionalOrderModelForOrderDetailsFromPushNotification;
  int customerChatUnReadMessagesCount = 0;
  ...
  OrderModel get optionalOrderModel => _optionalOrderModelForOrderDetailsFromPushNotification;
  ...
  Future<void> getOrderDetails(int orderID,[bool isNeedToSetOrderModel = false]) async {
    _orderDetailsModel = null;
    Response response = await orderRepo.getOrderDetails(orderID,isNeedToSetOrderModel);
    if(response.statusCode == 200) {
      _orderDetailsModel = [];
      customerChatUnReadMessagesCount = response.body['un_read_count_normal_chat']??0;
      response.body["order_details"].forEach((orderDetails) => _orderDetailsModel.add(OrderDetailsModel.fromJson(orderDetails)));
      if(isNeedToSetOrderModel){
        _optionalOrderModelForOrderDetailsFromPushNotification = OrderModel.fromJson(response.body["order"]);
      }
    }else {
      ApiChecker.checkApi(response);
    }
